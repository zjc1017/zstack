...
static void SerialApp_Cmd_Resp(void);
...
UINT16 SerialApp_ProcessEvent( uint8 task_id, UINT16 events )
{
  ...
  if ( events & SYS_EVENT_MSG )
  {
    ...
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( SerialApp_TaskID )) )//获取消息
    {
  ...
  if ( events & SERIALAPP_CMD_RESP_EVT)
  {
    SerialApp_Cmd_Resp();
    return ( events ^ SERIALAPP_CMD_RESP_EVT);
  }
...
void SerialApp_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  else
  {
    ...
    {
      //Serial Test
       #define  Serial_T_Cnt 11
       byte Serial_T[Serial_T_Cnt];
       Serial_T[0]='S';
       Serial_T[1]='e';
       Serial_T[2]='r';
       Serial_T[3]='i';
       ...
       Serial_T[9]='s';
       Serial_T[10]='t';
       HalUARTWrite( SERIAL_APP_PORT,Serial_T, Serial_T_Cnt);
    }
...
static int get_val_by_id(int id)
{
	int val;
	switch (id) {
    ...
    */
{
 // decode
	...
	int id;
	char buf[32];
  struct zb_get_req req;
  struct zb_get_rsp rsp;
  struct zb_item_pair items[10];

	rv = zb_decode_get_req(&req, (char*)pkt->cmd.Data + 1, pkt->cmd.DataLength );
  //decode test
  //AF_DataRequest(&SerialApp_TxAddr,
                                           //(endPointDesc_t *)&SerialApp_epDesc,
                                            //SERIALAPP_CLUSTERID1,
	...
	switch (req.hdr.cmd) {
		case ZB_ID_GET_REQ:
			...
			// new item pair array
			// items = malloc(sizeof(item_pair) * req.ids.count);

			// travel
			for (i = 0; i < req.ids.count; ++i) {
				id = req.ids.id[i];
				items[i].id = id;
				items[i].val = get_val_by_id(id);
			}

			rsp.pairs.count = req.ids.count;
			rsp.pairs.pairs = items;

			// rv = zb_encode_get_req(&req, buf, sizeof(buf));
      rv = zb_encode_get_rsp(&rsp, buf + 1, sizeof(buf) - 1);
			if (rv <= 0) {
				break;//return;
			}
      buf[0] = ++SerialApp_TxSeq;//0x0F
      AF_DataRequest(&SerialApp_TxAddr,
                                           (endPointDesc_t *)&SerialApp_epDesc,
                                            SERIALAPP_CLUSTERID1,
                                            rv+ 1 , (unsigned char *)buf,
                                            &SerialApp_MsgID, 0, AF_DEFAULT_RADIUS);
			//send(buf, rv);
      HalLedSet ( HAL_LED_3, HAL_LED_MODE_ON); //indicate receive Serial message
			break;
		default:
			break;
	}
  ...
  case SERIALAPP_CLUSTERID2:
    HalLedSet ( HAL_LED_3, HAL_LED_MODE_ON); //indicate receive response
    HalLedSet ( HAL_LED_2, HAL_LED_MODE_ON );  
    ...
    default:
    HalLedSet ( HAL_LED_3, HAL_LED_MODE_ON); //indicate receive default
...
static void SerialApp_Resp(void)
{
  ...
  {
    //osal_set_event(SerialApp_TaskID, SERIALAPP_CMD_RESP_EVT);
  }

}
/*********************************************************************
 * @fn      SerialApp_Cmd_Resp
 *
 * @brief   RESPOSE TO CMD.
 *
 * @param   none
 *
 * @return  none
